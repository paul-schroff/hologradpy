...
def shear_rotate(field: Tensor, angle: float | Tensor) -> Tensor:
    ...
    Decomposes ``R = Sx(-tan(theta/2)) . Sy(sin theta) . Sx(-tan(theta/2))``, each shear
    an exact band-limited per-line shift. The rotation is area-preserving (so
    power-conserving) and differentiable w.r.t. the field. Valid for ``|angle| < pi/2``;
    for larger rotations pre-rotate by a multiple of 90 degrees (exact transpose/flip)
    and apply the residual here.

    ``angle`` may be a Python float (fast path) or a 0-d :class:`torch.Tensor`, in which
    case the rotation is also differentiable w.r.t. the angle (so it can be a learnable
    parameter) -- the shears are then computed with torch ops and the ``angle == 0``
    short circuit is skipped so a gradient still flows at 0.
    """
...
def _shear_rotate_tensor(field: Tensor, angle: Tensor) -> Tensor:
    ...
    0-d tensor angle. No ``angle == 0`` short circuit, so a gradient flows even at zero
    rotation."""
    height, width = field.shape[-2:]
