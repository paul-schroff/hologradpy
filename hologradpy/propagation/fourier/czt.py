...
def _czt_forward_last(field: Tensor, omega: Tensor) -> Tensor:
    ...
    ``X_k = sum_n field[n] * exp(-i * omega_k * (n - N//2))`` at the uniformly
    spaced sample frequencies ``omega`` (rad/sample), exactly.
    """
...
def _czt_adjoint_last(samples: Tensor, omega: Tensor, length: int) -> Tensor:
    ...
    ``omega`` grid back to a ``length``-point input axis,
    ``x_n = sum_k samples[k] * exp(+i * omega_k * (n - N//2))``.
    """
    points = omega.shape[0]
...
class ChirpZZoom(FourierBase):
    ...
    The scale and shift come from the chirp-z (Bluestein) transform applied
    separably along each axis at the :func:`get_zoom_frequency_grid` sample
    points; the (optional) rotation is a 3-shear FFT rotation of the input,
    using ``F(R k) = FT(f o R)(k)``. Unlike the KbNufft this is the *exact* DFT
    at those points (``is_gridded=True``), so its power is correct -- no
    interpolation/normalization fudge. Fully differentiable.
    """
