...
class CameraSensor(OpticsModule):
    ...
    ``MockCamera``, without the point-spread convolution): the per-pixel optical
    power ``|E|^2 * pixel_area`` is turned into a photon count via the photon
    energy (from the field's wavelength) and the exposure time, scaled by the
    quantum efficiency and an optional neutral-density filter, optionally given a
    Poisson read-noise floor, multiplied by the gain, clipped to the full-well
    capacity (saturation), scaled to the bit-depth full scale, and optionally
    floored to integer counts.

    ``forward`` returns a *real* pixel image ``(*batch, H, W)`` -- the wavelength
    axis is summed (a monochrome sensor). It is therefore terminal: it does not
    return a :class:`ComplexAmplitude` and cannot be chained further.

    Two modes:
    - Default (``add_noise=True, quantize=True``): realistic capture -- Poisson
      read noise plus an integer bit-depth floor. Stochastic and not
      differentiable.
    - ``add_noise=False, quantize=False``: the deterministic expected ADU, fully
      differentiable (the only non-smooth step is the full-well clip, like a
      ReLU). Use this for gradient-based calibration / optimization.
    """
