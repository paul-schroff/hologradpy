...
class RasterCalibrator(WavefrontCalibratorBase):
    ...
    def auto_camera_roi(
        ...
        spot_position, _, calibration_image = get_diffraction_spot_position(
            self.slm,
            self.camera,
            linear_phase_tilt,
    ...
    def measure_intensity(
        ...
        """
        spot_position_x, spot_position_y = self.auto_camera_roi(
            linear_phase_tilt, camera_roi_size
        )[0]
        ...
        for i, superpixel_slice in enumerate(slicer.slices):
            ...
            superpixel_power[superpixel_slice] += np.sum(
                camera_images[i, ...]
    ...
    def measure_phase(
        ...
        if measured_intensity is None:
            ...
            beam_radius, shift_x, shift_y = self.fit_gaussian_beam(
                measured_intensity
            )
