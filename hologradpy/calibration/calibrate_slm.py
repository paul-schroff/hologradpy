...
def get_aperture_indices(
    nx, ny, x_start, x_stop, y_start, y_stop, aperture_width, aperture_height
):
    """
    ...
    """
    idx_start_x = np.floor(np.linspace(x_start, x_stop - aperture_width, nx)).astype(
        "int"
    )
    ...
    idx_start_y = np.floor(np.linspace(y_start, y_stop - aperture_height, ny)).astype(
        "int"
    )
...
def measure_slm_wavefront(
    ...
    phi_int[
        slm_idx[0][n_centre] : slm_idx[1][n_centre],
    ...
    phi_int[
        slm_idx[0][n_centre_ref] : slm_idx[1][n_centre_ref],
    ...
    laser_intensity_upscaled = pt.load_filter_upscale(
        laser_intensity_measured, npix, 1, filter_size=pms_obj.i_filter_size
    )
    laser_intensity_upscaled = np.pad(
        laser_intensity_upscaled, ((0, 0), (border_x, border_x))
    )
    ...
    for i in range(roi_n**2):
        ...
        norm[i] = p_max / np.sum(
            laser_intensity_upscaled[
                slm_idx[0][ii] : slm_idx[1][ii], slm_idx[2][ii] : slm_idx[3][ii]
            ]
        )
        ...
        masked_phase[
            slm_idx[0][ii] - pad : slm_idx[1][ii] + pad,
        ...
        aperture_power[i] = (
            np.mean(img[:, :, i]) * aperture_width**2 / aperture_width_adj[i] ** 2
        )
    ...
    for i in range(roi_n**2):
        ...
        popt, pcov = opt.curve_fit(
            fit_sine.fit_sine, x_data, img_i.ravel(), p0, bounds=bounds, maxfev=50000
        )
    ...
    pad_roi = (
        (roi_min_x, aperture_number - roi_n - roi_min_x),
