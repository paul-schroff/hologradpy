...
def asm(e_in: torch.Tensor,
    ...
    :param theta2: Propagation phase from lens to camera. If not 
        provided it is assumed ``theta2 = theta1``.
    :param bool shift: Perform FFT shift?
...
class ASM:
    ...
    def __init__(self,
        ...
        self.theta1 = torch.tensor(
            np.exp(1j * pd1 * 
                   np.sqrt(pms_obj.k ** 2 - k_x ** 2 - k_y ** 2 + 0j)),
            dtype=dtype,
            device=device)
        ...
        phi_l = pt.doublet(x_l,
                           ...
                           pms_obj.r2,
                           pms_obj.r3)
        a_l = pt.circ_mask_xy(x_l,
                              y_l,
                              0,
                              0,
                              extent_lens / 2)
        self.e_lens = torch.tensor(a_l * np.exp(1j * phi_l),
                                   dtype=dtype,
                                   device=device)

        # quadratic phase to cancel focal length mismatch
        range_q = slm_disp_obj.slm_size[0] / 2
        x_q_native = np.arange(-range_q,
                               range_q,
                               slm_disp_obj.pitch)
        X_q_native, Y_q_native = np.meshgrid(x_q_native,
                                             x_q_native)
        phi_corr_native = -pt.slm_phase_doublet(X_q_native,
                                                Y_q_native,
                                                pms_obj.k,
                                                xf,
                                                xf,
        ...
        phi_corr = -pt.slm_phase_doublet(x_corr,
                                         y_corr,
                                         xf,
                                         xf,
                                         pd1,
...
def loss_fn_fid(
    ...
    phi_out = e_out.angle()
    overlap = tt.sum(
        signal * a_out * tt.sqrt(i_tar) * tt.cos(phi_out - phi_tar)
    )
    overlap = (
        overlap / (tt.sqrt(tt.sum(i_tar) * tt.sum((a_out * signal) ** 2)))
    )
