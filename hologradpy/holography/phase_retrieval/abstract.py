...
class PhaseRetrievalData(SaveableRecord):
    ...
    def load_step(self, iteration: int, directory: str | os.PathLike) -> NDArray:
        ...
        Args:
            ...
            directory: Where the recorded steps live, supplied by the caller so a
                moved set still loads.
        ...
        with RetrievalStepStore.open(Path(directory) / RETRIEVAL_STEPS_NAME) as store:
    ...
    def replay(
        ...
        Args:
            iteration: One of :attr:`step_iterations`.
            directory: Where the recorded steps and the checkpoint live.
        ...
        if model is None:
            ...
            model = load_optical_system(Path(directory) / self.model_checkpoint)

        fraction = self.load_step(iteration, directory)

        # The parameter goes back exactly as it was, which is why it rather than the
        # phase is what a step holds.
...
class PhaseRetrieverBase:
    ...
    def __init__(
        ...
        self.loss_function: Callable | None = None
