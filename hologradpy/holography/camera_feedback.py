...
def camera_calibration(
    ...
    phase_retrieval_obj = PhaseRetrieval(
        slm_obj, n_iter=n_iter, i_tar=holo.i_tar, signal_region=holo.sig_mask
    )
    ...
    IoutCal = holo.i_tar[
        centre - n_crop : centre + n_crop, centre - n_crop : centre + n_crop
    ]
    ...
    imgCaltf = cv.warpAffine(
        imgCal,
        itf,
        (slm_obj.npix_pad * slm_obj.pix_res, slm_obj.npix_pad * slm_obj.pix_res),
    )
...
def camera_feedback(
    ...
    sig_mask_tf = cv.warpAffine(
        phase_retrieval_obj.signal, tf, (cam_obj.res[1], cam_obj.res[0])
    )
        ...
        while n_vtx > 0:
            ...
            vtx = pt.detect_vortices(
                ...
                (
                    ...
                    / np.max(phase_retrieval_obj.i_tar)
