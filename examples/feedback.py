...
from hologradpy.analysis import error_metrics as m
from hologradpy.calibration import calibrate_slm as clb
from hologradpy import torch_functions as tfn

from examples.experiment import Params, Camera, SlmDisp

# %%
# Here, we determine which computing hardware to use (CPU or GPU) and create instances
# from our custom hardware classes.

device = tfn.check_device(verbose=True)  # Check for GPU

pms_obj = Params()
cam_obj = Camera(
    np.array([960, 1280]), 3.75e-6, bayer=True
...
threshold_vtx = (
    0.05  # Vortices are only detected in regions brighter than threshold (1 is maximum)
)
...
measure_slm_intensity = (
    False  # Measure the constant intensity at the SLM (laser beam profile)?
)
measure_slm_phase = False  # Measure the constant phase at the SLM?

# %%
# Parameters for the initial SLM phase guess, the target light potential and the signal
# region:
...
linear_phase = np.array(
    [phase_angle + 2, phase_angle - 2]
)  # Linear term of the initial phase guess
...
extent_lens = (
    npix_pad * slm_disp_obj.pitch
)  # Spatial extent of computational lens plane [m]
...
# -----------------------------------------------------
# Measuring the constant intensity and phase at the SLM is crucial for accurate
# experimental results - see the supplementary information of our publication for
# details.
if measure_slm_intensity is True:
    i_path = clb.measure_slm_intensity(
        slm_disp_obj, cam_obj, pms_obj, 30, 32, 10000, 256, 300
    )
    pms_obj.i_path = i_path
if measure_slm_phase is True:
    phi_path = clb.measure_slm_wavefront(
        slm_disp_obj,
        cam_obj,
        pms_obj,
        30,
...
# %%
# Using the functions above, this is our constant field at the SLM after upscaling it to
# the native resolution of the SLM:
#
# .. image:: images/constant_slm_field.png

# %% Defining the target light potential
# -----------------------------------
# The ``patterns.Hologram`` class contains pre-defined patterns from our publication. It
# creates
#
...
#
# Feel free to define the arrays above yourself - using the ``patterns.Hologram`` class
# is not mandatory.
...
# phase term. The quadratic phase term depends on the size and the aspect ratio of the
# target pattern while the linear term depends on the position of the pattern with
# respect to the optical axis. The initial phase guess defined here looks like this:
#
# .. image:: images/init_phase.png :width: 400

# %% Creating a virtual SLM object
# -----------------------------
# This is a digital twin of the experimental Fourier holography setup. The ``forward``
# method of ``VirtualSlm`` takes an SLM phase pattern, models pixel crosstalk on the SLM
# and the propagation of light from the SLM to the camera. It returns the electric field
# at the image plane.
...
img_pitch = (
    pms_obj.wavelength * pms_obj.fl / (slm_disp_obj.pitch * slm_disp_obj.res[0] * 2)
)
xf = -256 * img_pitch  # ToDO: Explain this.

# Create virtual SLM object
slm_obj = tfn.VirtualSlm(
    slm_disp_obj,
    pms_obj,
    holo.phi_init,
    npix_pad,
    ...
    pix_res=pix_res,
    propagation_type=propagation_type,
    extent_lens=pms_obj.lens_aperture,
    pd1=pd1,
    pd2=pd2,
    xf=xf,
    device=device,
...
# Camera calibration
# ------------------
# Here, we calculate the affine transformation matrix between camera coordinates and
# image plane coordinates. This is important to compare the simulated light potential to
...
if calc_transform is False:
    ...
    tf, itf = tfn.camera_calibration(
        ...
        save=True,
        exp_time=1000,
        checkerboard_rows=16,
        checkerboard_columns=12,
        checkerboard_square_size=30,
    )
...
#
# Note that the zeroth-order diffraction spot is now located in the center of the
# computational image plane on the right hand side.
...
#
# Before running the camera feedback algorithm, we set the phase of the virtual SLM ,
# ``slm_obj``, with the initial phase guess. The phase pattern of ``slm_obj`` might have
# been modified by the ``torch_functions.camera_calibration`` function.

phase_retrieval_obj = tfn.PhaseRetrieval(
    slm_obj,
    n_iter=int(iter_cg[0]),
    i_tar=holo.i_tar,
...
output = tfn.camera_feedback(
    ...
    iter_fb=iter_fb,
    iter_cg=iter_cg,
    detect_vortices=detect_vortices,
    threshold_vtx=threshold_vtx,
    n_save=n_save,
    n_avg=n_frames_avg,
    exp_time=exp_time,
    fb_blur=feedback_blur,
    alpha=alpha,
    convergence=convergence,
    iter_convergence=iter_plot,
    path=path,
)
...
#
# Here, we only show the signal region on the camera. The experimental errors in the
# camera image are greatly reduced after 10 camera feedback iterations with 50 CG
# iterations each:
#
...
im20 = axs2[0].imshow(
    p.crop(holo.i_tar, npix // 2) / np.max(holo.i_tar), cmap="turbo", extent=extent2
)
...
plt.imshow(
    diff_img,
    cmap="seismic",
    vmin=-np.max(np.abs(diff_img)),
    vmax=np.max(np.abs(diff_img)),
)

# %%
# We can investigate the convergence of the phase retrieval algorithm in-between
# feedback iterations by saving intermediate phase patterns, displaying them on the SLM
# and capturing the resulting camera image. This allows us to see when the rms error of
# the camera image converges to determine the number of CG iterations needed per
# feedback iteration.
...
if convergence is True:
    ...
    for i in range(len(iter_plot)):
        plt.figure("rmse")
        x = np.linspace(
            iter_plot[i] - 1 + 1 / iter_cg[iter_plot[i] - 1],
            iter_plot[i] - 1 + n_conv_sv[i] * n_save / iter_cg[iter_plot[i] - 1],
            n_conv_sv[i],
        )
    ...
    plt.figure("rmse")
    plt.plot(
        np.arange(1, iter_fb + 1),
        rmse * 100,
        "k*",
        label="RMS within 50% of max. intensity",
    )
    plt.legend()
