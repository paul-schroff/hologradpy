...
This script sets constant experimental parameters and implements camera and SLM drivers.
You will have to do this for your own hardware.
"""

import time
import numpy as np
from hologradpy.hardware import hardware as hw
...
# -------------------------------------------------------------
#
# To start off, we write our own subclass of ``hardware.ParamsBase`` which sets some
# experimental parameters and other constants needed in this script.
...
# -----------------------------------
# This module provides the ``hardware.CameraBase`` class and the ``hardware.SlmDisp``
# class to interface with the camera and the SLM. You will have to write your own
# subclasses for the specific devices you are using. Here, we defined the subclasses
# ``Camera`` and ``SlmDisp`` to interface with a MatrixVision BlueFox 3 camera and a
# Hamamatsu SLM. Make sure you implement all abstract methods of ``hw.CameraBase`` and
# ``hw.SlmBase`` in your own subclasses.
...
class SlmDisp(hw.SlmBase):
    def __init__(self, res, pitch, calib=None, delay=0.2, dx=0, dy=0):
        ...
        )  # Index of max_phase in lut
        self.lut = self.lut[: self.idx_lut]
        # Path to Hamamatsu SLM correction pattern.
        self.cal_path = (
            pms_obj.data_path
            + "deformation_correction_pattern/CAL_LSH0802439_"
            + "{:.0f}".format(np.around(pms_obj.wavelength * 1e9, decimals=-1))
            + "nm.bmp"
        )
    ...
    def display(self, phi_slm):
        ...
        if slm_pad_x == 0 and slm_pad_y == 0:
            ...
            phi_zeros[
                slm_pad_y - self.dy :, slm_pad_x - self.dx : -slm_pad_x - self.dx
            ] = phi_disp
        elif -slm_pad_x - self.dx == 0:
            phi_zeros = np.zeros((slm_res_y, slm_res_x))
            phi_disp = slm_norm * phi_slm / (2 * np.pi)
            phi_zeros[
                slm_pad_y - self.dy : -slm_pad_y - self.dy, slm_pad_x - self.dx :
            ] = phi_disp
        else:
            phi_zeros = np.zeros((slm_res_y, slm_res_x))
            phi_disp = slm_norm * phi_slm / (2 * np.pi)
            phi_zeros[
                slm_pad_y - self.dy : -slm_pad_y - self.dy,
                slm_pad_x - self.dx : -slm_pad_x - self.dx,
            ] = phi_disp
